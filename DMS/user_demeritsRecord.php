...
<article><br><br>
	...
	<?php 
			...
			$sql = "SELECT * FROM student WHERE dorm = '$dorm' AND dorm_type = '$type' AND request='Pending' AND seen = 0";
		...
		<?php 
			...
			$sql = "SELECT * FROM lcard WHERE dorm = '$dorm' AND dorm_type = '$type' AND status='Pending' AND seen = 0";
			$result = mysqli_query($con, $sql);
	...
	<th>Room</th>
		...
		<?php
			if(isset($_POST['submit']))
			{
				...
				$sql = "SELECT * FROM student where dorm='$dorm'";
			$result=mysqli_query($con,$sql);
								while($rows=mysqli_fetch_array($result))
								{
				...
				$qry="insert into demerits (fname,lname,dorm_number,room_number,dorm_type,dorm) values('$fname','$lname','$area','$room','$type','$dorm')";
				$result=mysqli_query($con,$qry);
				if($result)
			...
			function displayimage()
			{
				...
				$qry="select * from demerits where dorm='$area' and dorm_type='$type'";
				$str="";
				$result=mysqli_query($con,$qry);
				while($rows=mysqli_fetch_array($result))
				{
