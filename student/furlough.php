	...
	<th>Arrival</th>
		...
		<?php
			...
			if(isset($_POST['submit']))
			{
				...
				{	
					$qry="insert into furlough (name,room_number, destination, departure) values ('$name','$room','$destination',Now())";
					$result=mysqli_query($con,$qry);
					if($result)
					{
						echo 
						"<script>window.alert('SUCCESSFUL!')
						document.location = 'furlough.php'</script>";
					}
					else{
			...
			function displayimage()
			{
				...
				$qry="select * from furlough where name='$name';";
				$str="";
				$result=mysqli_query($con,$qry);
				while($rows=mysqli_fetch_array($result))
				{
